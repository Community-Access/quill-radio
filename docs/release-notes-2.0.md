...
- **A whole new way to browse -- one unified tree of every source.** You asked (thanks, Kelly) to be able to pick a source like SomaFM and just see its stations, without typing a search first -- and for browsing to feel like its own thing, not a search form. So **Browse Stations** is now its own dedicated, search-free window: a single tree whose branches are the sources. You expand a branch and its stations appear on the spot; press **Enter** to play the highlighted one, and **Shift+F10** (or right-click) opens a full menu -- Play/Stop, Add or Remove Favorite, Copy the stream link, Open the station's website -- with a **Refresh** to re-pull a source. It reads like a media library you wander, not a form you fill in. The branches are **Popular Stations**, **Weather / NOAA**, **ACB Media**, **NFB Radio**, **SomaFM**, **TuneIn** (its real folder tree), **Community M3U (Music Genres)**, and the **Xiph / Icecast Directory** -- described below. **Search Stations...** stays a separate window with the familiar name/tag/country box.
  - **Favorites** is the first branch, at the very top -- your own saved folders and streams, built instantly, so you can jump straight to one without passing the directories. Right-click a source folder for **Add All Stations to Favorites**, and note the **Play button reads "Stop"** while the highlighted station is the one playing.
  - **Weather / NOAA** lists **NOAA Weather Radio (NWR)** internet re-streams -- the actual NWR transmitters, named with their call sign and frequency ("NOAA Weather Radio KHB36 Manassas 162.550 MHz") -- browse, play, and favorite them like anything else. NOAA has no audio API of its own (NWR is over-air VHF), so these are the online re-streams listeners share; Quill Radio finds the genuine ones by name.
  ...
  - **TuneIn** browses TuneIn's own directory as a real **folder tree**: pick **TuneIn** and its top-level folders appear -- **Music, Talk, Sports, Local Radio, By Location, By Language**, and more. Open a folder to reveal its sub-folders and stations (it loads as you open it), collapse it to step back, and press **Enter** on a station to play it right there. No search required -- just wander in.
...
- **Browsing and searching are now clearly separate.** The Station menu splits the two: **Browse Stations...** opens the unified source tree above, while **Search Stations...** opens the field-based window with your cursor already in the search box. No more one screen that tried to be both -- browsing has its own delightful path, and search stays search.
