...
- **Browse a source without searching -- and several new sources.** You asked (thanks, Kelly) to be able to pick a source like SomaFM and just see its stations, without typing a search first. Now you can: the Browse Stations **Category** list lets you pick a source and the stations appear on their own. Alongside your Favorites you now get **Popular Stations**, **SomaFM**, **ACB Media**, **TuneIn**, **Music Genres**, and the **Xiph Directory** -- each described below. (**NFB Radio** is a single stream, so it lives on the **Station menu** next to ACB Media rather than in the browse list.)
...
- **Browsing and searching are now clearly separate.** The Station menu splits the two: **Browse Stations** is a submenu of sources (Popular Stations, ACB Media, SomaFM, TuneIn, Music Genres, Xiph Directory) and each one opens Browse Stations straight to that source, while **Search Stations...** opens the same window with your cursor already in the search box. No more one screen that tried to be both -- browsing has its own path, and search stays search.
